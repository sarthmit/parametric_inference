...
from score_wrapper import QuadNoiseSchedule
from posterior_utils import posterior_mean_full_gaussian, posterior_linear_regression
...
def train(args, test_data, dataset, lengths):
    ...
    eval_func, ensemble_eval_func, eval_mode = get_evaluation_helpers(args.experiment, n_mixtures=args.n_mixtures)
    ...
    with open(f'{path}/log.txt', 'w') as f:
        for it in tqdm_batch:
            ...
            '''
            loss = train_step(model, optimizer, dataset, dataset.conditional_fn, likelihood, prior_args, args, noise_schedule, it)
            ...
            if it % args.save_iters == 0:
                ...
                for di, dt in enumerate(test_data):
                    approx_posterior, test_metrics = eval_step(dt, model, dataset.conditional_fn, likelihood,
                    ...
                    f.write(f'Iteration: {it} | Dimensionality: {lengths[di]} | {test_metrics}')
                    print(test_metrics)
...
def prior_metric(eval_path, prior, test_data, lengths, likelihood, conditional_fn, eval_func, ensemble_eval_func, eval_mode, num_eval_samples):
    '''
    ...
    with open(f'{eval_path}/prior.txt', 'w') as f:
        for di, dt in enumerate(test_data):
            ...
            # torch.save(parameters, f'{eval_path}/prior-parameters_{lengths[di]}.pt')
            f.write(f'Dimensionality: {lengths[di]} | {prior_metrics}')

def optimization_metric(eval_path, optimizer, test_data, lengths, likelihood, conditional_fn, eval_func, ensemble_eval_func, eval_mode, num_eval_samples):
    '''
    ...
    with open(f'{eval_path}/optimization.txt', 'w') as f:
        for di, dt in enumerate(test_data):
            ...
            # torch.save(parameters, f'{eval_path}/optimization-parameters_{lengths[di]}.pt')
            ...
            f.write(f'Dimensionality: {lengths[di]} | {optimization_metrics}')

def langevin_metric(eval_path, langevin, test_data, lengths, likelihood, conditional_fn, eval_func, ensemble_eval_func, eval_mode, num_eval_samples):
    '''
    ...
    with open(f'{eval_path}/langevin.txt', 'w') as f:
        for di, dt in enumerate(test_data):
            ...
            parameters, parameters_last, parameters_single = langevin.chain(train_x, train_y, mask, params_mask, (params[1],), likelihood, conditional_fn, num_samples = num_eval_samples, samples_per_chain = num_eval_samples, show_tqdm=True)
            ...
            f.write(f'Dimensionality: {lengths[di]} | {langevin_metrics}')
...
def hmc_metric(eval_path, hmc, test_data, lengths, likelihood, conditional_fn, eval_func, ensemble_eval_func, eval_mode, num_eval_samples):
    '''
        Get langevin metrics for the baseline methods
    '''
    with open(f'{eval_path}/hmc.txt', 'w') as f:
        for di, dt in enumerate(test_data):
            ...
            parameters, parameters_last, parameters_single = hmc.chain(train_x, train_y, mask, params_mask, (params[1],), likelihood, conditional_fn, num_samples = num_eval_samples, samples_per_chain = num_eval_samples, show_tqdm=True)
...
def true_posterior_metric(eval_path, test_data, lengths, likelihood, conditional_fn, eval_func, ensemble_eval_func, eval_mode, num_eval_samples, args):
    '''
        Get prior metrics for the baseline methods
    '''

    with open(f'{eval_path}/true_posterior.txt', 'w') as f:
        for di, dt in enumerate(test_data):
            train_samples, val_samples, params, mask, params_mask = dt
            ...
            if args.experiment == 'gaussian':
                true_posterior = posterior_mean_full_gaussian(train_x, mask, params[-1])
            elif args.experiment == 'linear_regression':
                true_posterior = posterior_linear_regression(train_x, train_y, mask, args.beta, device=device)
            else:
                break
            parameters = [true_posterior.sample() for _ in range(num_eval_samples)]

            metrics = compute_metrics(parameters, x, y, mask, params_mask, (params[1],), likelihood, conditional_fn, eval_func, ensemble_eval_func, name='Posterior', evaluation_mode=eval_mode)
            metrics = metrics_to_string(metrics)
            print(metrics)
            # f.write(f'Dimensionality: {lengths[di]} | {metrics}')

def true_baseline(args, test_data, dataset, lengths, num_test_datasets = 100):
    path = get_experiment_name(args)
    eval_path = f'Baseline/{path}'
    os.makedirs(eval_path, exist_ok=True)
    in_dim, params_dim = get_params_dim(args)
    likelihood = get_likelihood(args.experiment, args.p_var)
    eval_func, ensemble_eval_func, eval_mode = get_evaluation_helpers(args.experiment, args.n_mixtures)
    true_posterior_metric(eval_path, test_data, lengths, likelihood, dataset.conditional_fn, eval_func, ensemble_eval_func, eval_mode, args.eval_samples, args)

def baseline(args, test_data, dataset, lengths, num_test_datasets = 100):
    path = get_experiment_name(args)
    eval_path = f'Baseline/{path}'
    os.makedirs(eval_path, exist_ok=True)
    in_dim, params_dim = get_params_dim(args)
    ...
    prior_metric(eval_path, prior, test_data, lengths, likelihood, dataset.conditional_fn, eval_func, ensemble_eval_func, eval_mode, args.eval_samples)

    optimizer = Optimization(params_dim, device)
    optimization_metric(eval_path, optimizer, test_data, lengths, likelihood, dataset.conditional_fn, eval_func, ensemble_eval_func, eval_mode, args.eval_samples)

    langevin = Langevin(params_dim, device)
    langevin_metric(eval_path, langevin, test_data, lengths, likelihood, dataset.conditional_fn, eval_func, ensemble_eval_func, eval_mode, args.eval_samples)

    hmc = HMC(params_dim, device)
    hmc_metric(eval_path, hmc, test_data, lengths, likelihood, dataset.conditional_fn, eval_func, ensemble_eval_func, eval_mode, args.eval_samples)

def eval(args, test_data, dataset, lengths):
    path = get_experiment_name(args)
    saved_path = f'results/{path}'
    eval_path = f'Evaluation/{path}'
    ...
    state = torch.load(f'{saved_path}/model_last.pt')['state_dict']
    ...
    with open(f'{eval_path}/log.txt', 'w') as f:
        for di, dt in enumerate(test_data):
            approx_posterior, test_metrics = eval_step(dt, model, dataset.conditional_fn, likelihood,
                                    ...
                                    it=None, eval_samples=args.eval_samples, log_wandb=args.wandb, num_integration_steps = 100)
            print(test_metrics)
            f.write(f'Dimensionality: {lengths[di]} | {test_metrics}')

def analysis(args, test_data, dataset, lengths):
    path = get_experiment_name(args)
    saved_path = f'results/{path}'
    eval_path = f'Analysis/{path}'
    ...
    with open(f'{eval_path}/log.txt', 'w') as f:
        for di, dt in enumerate(test_data):
            dt = list(dt)
            for n_samples in [64, 96, 128]:
                dt[3] = torch.ones_like(dt[3])
                dt[3][:, :n_samples] = 0
                approx_posterior, test_metrics = eval_step(dt, model, dataset.conditional_fn, likelihood,
                                        eval_func, ensemble_eval_func, eval_mode, num_classes=args.num_classes,
                                        it=None, eval_samples=args.eval_samples, log_wandb=args.wandb, num_integration_steps = 100)
                print(test_metrics)
                f.write(f'Sequence Length: {n_samples} | Dimensionality: {lengths[di]} | {test_metrics}')
...
if __name__ == '__main__':
    ...
    if args.mode == 'eval':
        set_seed(args.seed)
        analysis(args, test_data, dataset, lengths)
    elif args.mode == 'baseline':
        set_seed(args.seed)
        baseline(args, test_data, dataset, lengths, num_test_datasets)
        ...
        set_seed(args.seed)
        train(args, test_data, dataset, lengths)
    elif args.mode == 'true':
        set_seed(args.seed)
        true_baseline(args, test_data, dataset, lengths, num_test_datasets)